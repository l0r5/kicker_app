/**
 * App Routes
 */
const String ROUTE_HOME = '/home';
const String ROUTE_LOGIN = '/login';
const String ROUTE_REGISTER = '/register';
const String ROUTE_SINGLE_MATCH = '/single_match';

/**
 * Style Consts
 */

//COLORS
const int PRIMARY_COLOR = 0xff20a120;
const int ACCENT_COLOR = 0xffa12020;
const int RESULT_CARD_COLOR = 0xffffffff;

// RESULT CARD
const double RESULT_CARD_HEIGHT_SIZE_S = 80;
const double RESULT_CARD_HEIGHT_SIZE_M = 120;
const double RESULT_CARD_HEIGHT_SIZE_L = 160;
const double RESULT_CARD_HEIGHT_SIZE_XL = 190;
const double RESULT_CARD_HEIGHT_HEADER = 30;
const double RESULT_CARD_BORDER_RADIUS = 3.0;


